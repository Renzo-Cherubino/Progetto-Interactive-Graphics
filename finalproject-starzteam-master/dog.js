...
class Dog{
  constructor(){
    ...
    this.boxReference.position.set(0, 1, 0);
  ...
  drawBody() {
    const bodyGeometry = new THREE.BoxBufferGeometry(1.0*size, 1.2*size, 2.5  *size);
    this.body = new THREE.Mesh(bodyGeometry, this.skinMaterial);
    this.body.castShadow = true;
    this.body.receiveShadow = true;
    this.group.add(this.body);

    const tailGeometry = new THREE.BoxBufferGeometry(0.35*size, 0.5*size, 1.5*size);
    this.tail = new THREE.Mesh(tailGeometry, this.skinMaterial);
    this.tail.receiveShadow = true;
    this.tail.castShadow = true;

    this.tail.position.set(0*size, 0.06*size, -1.6*size);
    this.tail.rotation.set(rad(-15), 0, 0);
    this.group.add(this.tail);

    const tailEndGeometry = new THREE.BoxBufferGeometry(0.02*size, 0.23*size, 0.4*size);
  ...
  drawHead() {
    const headGeometry = new THREE.BoxBufferGeometry(0.9*size, 0.8*size, 0.7*size);
    const head = new THREE.Mesh(headGeometry, this.skinMaterial);
    head.receiveShadow = true;
    head.castShadow = true;

    head.position.set(0*size, 0.1*size,1.6*size);
    this.group.add(head);

    const noseGeometry = new THREE.BoxBufferGeometry(0.42*size, 0.5*size, 0.8*size);
    const nose = new THREE.Mesh(noseGeometry, this.skinMaterial);
    nose.receiveShadow = true;
    //nose.castShadow = true;
    nose.position.set(0*size, -0.1*size, 0.3*size);
    head.add(nose);

    const noseEndGeometry = new THREE.BoxBufferGeometry(0.3*size, 0.1*size, 0.15*size);
    const noseEnd = new THREE.Mesh(noseEndGeometry, this.blackMaterial);
    noseEnd.receiveShadow = true;
    noseEnd.position.set(0, 0.1*size, 0.4*size);
    nose.add(noseEnd);

    //Ears
    const leftEarGeometry = new THREE.BoxBufferGeometry(0.3*size, 0.45*size, 0.1*size);
    const leftEar = new THREE.Mesh(leftEarGeometry, this.whiteMaterial);
    leftEar.receiveShadow = true;
    //leftEar.castShadow = true;
    leftEar.position.set(0.3*size, 0.5*size, 0.1*size);
    leftEar.rotation.set(0, rad(40), 0);
    head.add(leftEar);

    const additionalLeftEarGeometry = new THREE.BoxBufferGeometry(0.4*size, 0.5*size, 0.08*size);
    ...
    additionalLeftEar.position.set(0, 0, -0.02);
    leftEar.add(additionalLeftEar);

    const rightEarGeometry = new THREE.BoxBufferGeometry(0.3*size, 0.45*size, 0.1*size);
    const rightEar = new THREE.Mesh(rightEarGeometry, this.whiteMaterial);
    rightEar.receiveShadow = true;
    //rightEar.castShadow = true;
    rightEar.position.set(-0.3*size, 0.5*size, 0.1*size);
    rightEar.rotation.set(0, rad(-40), 0);
    head.add(rightEar);

    const additionalRightEarGeometry = new THREE.BoxBufferGeometry(0.4*size, 0.5*size, 0.08*size);
    ...
    additionalRightEar.position.set(0, 0, -0.02);
    rightEar.add(additionalRightEar);

    //Eyes
    const leftEyeGeometry =new THREE.BoxBufferGeometry(0.1*size, 0.1*size, 0.05*size);
    const leftEye = new THREE.Mesh(leftEyeGeometry, this.blackMaterial);
    leftEye.receiveShadow = true;
    leftEye.position.set(0.25*size, 0.3*size, 0.4*size);
    head.add(leftEye);

    const rightEyeGeometry = new THREE.BoxBufferGeometry(0.1*size, 0.1*size, 0.1*size)
    const rightEye = new THREE.Mesh(rightEyeGeometry, this.blackMaterial);
    rightEye.receiveShadow = true;
    rightEye.position.set(-0.25*size, 0.3*size, 0.4*size);
    head.add(rightEye);
  }
  drawLegs() {

    //Right back Leg
    const rightBackLegGeometry = new THREE.BoxBufferGeometry(0.42*size, 0.6*size, 0.4*size);
    ...
    this.rightBackLeg.position.set(-0.4*size, -0.7*size, -0.8*size);
    this.group.add(this.rightBackLeg);

    const rightBackDownLegGeometry = new THREE.BoxBufferGeometry(0.4*size, 0.6*size, 0.4*size);
    ...
    this.rightBackDownLeg.position.set(0, -0.65*size, 0);
    this.rightBackLeg.add(this.rightBackDownLeg);

    const rightBackPawGeometry = new THREE.BoxBufferGeometry(0.4*size, 0.2*size, 0.45*size);
    ...
    rightBackPaw.position.set(0, -0.3*size, 0);
    this.rightBackDownLeg.add(rightBackPaw);

    //rightFrontLeg
    const rightFrontLegGeometry = new THREE.BoxBufferGeometry(0.42*size, 0.6*size, 0.4*size);
    ...
    this.rightFrontLeg.position.set(-0.4*size, -0.65*size, +1*size);
    this.group.add(this.rightFrontLeg);

    const rightFrontDownLegGeometry = new THREE.BoxBufferGeometry(0.4*size, 0.6*size, 0.4*size);
    ...
    this.rightFrontDownLeg.position.set(0, -0.65*size, 0);
    this.rightFrontLeg.add(this.rightFrontDownLeg);

    const rightFrontPawGeometry = new THREE.BoxBufferGeometry(0.4*size, 0.2*size, 0.45*size);
    ...
    rightFrontPaw.position.set(0, -0.3*size, 0.0);
    this.rightFrontDownLeg.add(rightFrontPaw);

    //Left Back Leg
    const leftBackLegGeometry = new THREE.BoxBufferGeometry(0.42*size, 0.6*size, 0.4*size);
    ...
    this.leftBackLeg.position.set(0.4*size, -0.65*size, -0.8*size);
    this.group.add(this.leftBackLeg);

    const leftBackDownLegGeometry = new THREE.BoxBufferGeometry(0.4*size, 0.6*size, 0.4*size);
    ...
    this.leftBackDownLeg.position.set(0, -0.65*size, 0);
    this.leftBackLeg.add(this.leftBackDownLeg);

    const leftBackPawGeometry = new THREE.BoxBufferGeometry(0.4*size, 0.2*size, 0.45*size);
    ...
    leftBackPaw.position.set(0, -0.3*size, 0);
    this.leftBackDownLeg.add(leftBackPaw);

    //Left Front Leg
    const leftFrontLegGeometry = new THREE.BoxBufferGeometry(0.42*size, 0.6*size, 0.4*size);
    ...
    this.leftFrontLeg.position.set(0.4*size, -0.7*size, +1*size);
    this.group.add(this.leftFrontLeg);

    const leftFrontDownLegGeometry = new THREE.BoxBufferGeometry(0.4*size, 0.6*size, 0.4*size);
    ...
    this.leftFrontDownLeg.position.set(0, -0.65*size, 0);
    this.leftFrontLeg.add(this.leftFrontDownLeg);

    const leftFrontPawGeometry = new THREE.BoxBufferGeometry(0.4*size, 0.2*size, 0.45*size);
    ...
    leftFrontPaw.position.set(0, -0.3*size, 0);
...
jump(speed, direction) {
    ...
    if(direction == "ahead"){
      ...
      if(counter == 0|| counter == 34){
        ...
        this.group.rotateOnAxis(new THREE.Vector3(1,0,0) , -Math.PI*(goingFastDog/8)*(1/16));
      }
      else if(this.group.rotation.x < 0) {
        this.group.rotateOnAxis(new THREE.Vector3(1,0,0) , +Math.PI*(goingFastDog/8)*(1/16))
      }
    ...
    if(counter == 0 || counter >=33){
      ...
      legRotation = Math.PI*(1/4)*(1/16) * goingFastDog;
    }
    else{
      legRotation = (-1)*Math.PI*(1/4)*(1/16) * goingFastDog;
    }
    ...
    if(direction == "ahead"){
      this.group.position.z = this.group.position.z + 0.11*goingFastDog;
    }
    else if (direction == "behind"){
      this.group.position.z = this.group.position.z - 0.11*goingFastDog;
    }
    else if(direction == "right"){
      this.group.position.x = this.group.position.x - 0.11*goingFastDog;
    }
